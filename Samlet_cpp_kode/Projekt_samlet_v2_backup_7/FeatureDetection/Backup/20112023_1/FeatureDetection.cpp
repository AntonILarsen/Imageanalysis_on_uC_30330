...
int main() {
	...
	while (1) {
		...
		int sizeThing_old = 100; //******************************************** NOT LIKE THIS****************************
		...
		if (c == '2')
		{
			...
			for (int ne = 0; ne < sizeThing; ne++) //Iterates through the new list of FAST features
			{
				int blablaIndex = 0;
				old_Score = INT_MAX;
				for (int ol = 0; ol < sizeThing_old; ol++) //Iterates through the old list of FAST features
				{
				...
				list[blablaIndex].active = (old_Score < 600) ? true : false;
				...
				list_Old[blablaIndex].active = (old_Score < 600) ? true : false;
