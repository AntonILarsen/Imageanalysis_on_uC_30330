...
int main() {
	...
	while (1) {
		...
		while (feat_n < NMAX)
		{
			...
			{
				feature_image.at<uchar>(Point(list[feat_n].x/delta_x, list[feat_n].y/delta_y)) = 255;
			}
...
int feature_detection(Mat latest_image, featurePoint list[NMAX], double threshold)
{
	...
	while (i < NMAX)
	{
		...
		if (list[i].active) //If the current index in the list of tracked features is 'active', it is currently being tracked, and shouldn't be updated
			list_length++;
		else if (feature_array_length > 1) //If the current index is not being tracked, we put the 'new' feature point coordinates into that index
		{
			int random_ind = rand() % (feature_array_length - 1); //Generate a random index for use of our list of indices of features
	...
	cout << "x, y in function: " << list[0].x << ",  " << list[0].y << '\n';
