...
int main() {
	...
	while (1) {
		...
		uint8_3_bgr_to_8_bit_gray(output, BW_image);
		GaussianBlur(BW_image, image_1, Size(5, 5), 0);
		...
		while (feat_n < NMAX)
		{
			...
			{
				BW_image.at<uchar>(Point(list_Old[feat_n].x, list_Old[feat_n].y)) = 255;
			}
			feat_n++;
		}
		...
		feature_detection2(image_1, list_Old, 20);

		for (int ne = 0; ne < NMAX; ne++) //Iterates through the new list of FAST features
		{
			int bestMatchIndex = 2;
			old_Score = INT_MAX;

			list_buffer[ne].active = false; //Clearing the buffer list to prevent false positive actives from lack of initialisation

			for (int ol = 0; ol < NMAX; ol++) //Iterates through the old list of FAST features
			{
				if (list_Old[ol].active) //Two if-statements for ekstra speed
				{
					int position_x = (int)list[ne].x;
					int position_y = (int)list[ne].y;
						...
						&& (position_y_old - delta_y > 0) && (position_y_old + delta_y < y_res))
					{
						...
						for (int x = 0; x < delta_x; x++)
						{
							...
							{
								score += (int)abs(image_1.at<uchar>(Point(position_x_old + x - half_delta_x, position_y_old + y - half_delta_x))
									- image_2.at<uchar>(Point(position_x + x - half_delta_x, position_y + y - half_delta_x)));
							}
						}
			...
			//cout << "Index ne: " << ne << "  best match index: " << bestMatchIndex << '\n';
			//cout << "list old x found: " << list_Old[bestMatchIndex].x << "best old y found : " << list_Old[bestMatchIndex].y << '\n';
		...
		for (int i = 0; i < NMAX; i++)
		{
			...
			list[i].active = list_buffer[i].active; //Make sure only features which were matched are active in each list
			list[i].score = list_buffer[i].score; //Copy the score (as it is a common value between old and new feature)
		}
		...
		feature_detection2(image_2, list, 20);
